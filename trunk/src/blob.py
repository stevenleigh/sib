...
class blob(object):
	...
	def read(self, key, full_file_name):
		...
		if not os.path.isfile(full_file_name):
			logging.error('blob does not exist.  path: %s' %(full_file_name))
			return None
		...
		logging.debug("Decoded file string: %s" %(decrypted_bytes))
		...
		for root, dirs, files in os.walk(directory):
			for name in files:
				...
				if compare_name == hash_prefix:
					return name
		logging.error('Couldn\'t find full hash for: %s' %hash_prefix)
		return None
